...
let test1 = max_cheese test_matrix
...
let best_value articles max_w =
  let n = Array.length articles in

  let rec best_value' acc i =
    if i >= n then
      acc
    else 
      let (name, price, weight) = articles.(i) in
      let ratio = price /. weight in
      let (acc_name, acc_price, acc_weight) = articles.(acc) in
      let acc_ratio = acc_price /. acc_weight in
      if ratio > acc_ratio then
        best_value' i (i+1)
      else
        best_value' acc (i+1)
  in
  let (name, price, weight) = articles.(best_value' 0 0)
  in
  let rec buy p w = 
    if w -. weight < 0. then
      p
    else
      buy (p+.price) (w-.weight)
  in
  buy 0. max_w
